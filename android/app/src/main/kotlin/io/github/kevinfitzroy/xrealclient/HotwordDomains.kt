 ...
 *
 * 新增一个行业 = 加一个 `val xxx = listOf(...)` + 挂到 [all]。挑**高频且 ASR 易听错**的英文专名,别灌字典。
 *
 * ⚠️ 两端同步:iOS `HotwordDomains.swift` 保持同一份内容。
 */
object HotwordDomains {
    ...
    val all: List<String> = linux + softwareDev + internet + ai + medicalAesthetics
}
