...
// go 下限 = 1.25.0:sing-box v1.13 本身只要 go1.24.7,但 gomobile 的 golang.org/x/mobile(下面
// tool 指令引入,bind 运行时必需)要 go1.25.0 → 实测构建下限被它抬到 1.25.0。本机 go1.25.x 缓存满足,
// 无需升级。build-ios.sh 自动用缓存的 go1.25.x toolchain 当 GOROOT(GOTOOLCHAIN=local)。
// 升级 sing-box 改下面 require 再 `go mod tidy`(build-ios.sh 缺 go.sum 时自动 tidy)。
go 1.25.0
