...
| iOS | **开发期(模拟器)**:`xcrun simctl get_app_container booted <bundle> data` 定位容器 → copy `hosts.json`/key 进 `Documents/`(= adb push 的 iOS 等价物,**仅模拟器**)。**真机:两个导入入口,都汇入同一条 `importConfig`**——① **分享单「Open in」**(AirDrop `.xrhosts` →「用 XrealPOC 打开」);② **原生 Host 配置页文档选择器**(列表页右上角齿轮入口进入,页内三个 `UIDocumentPickerViewController` 按钮 + 当前配置摘要)。Valet 产出**单个自包含 `.xrhosts` 文件**(JSON,与 staging 唯一差异 = **内联 key**),app 解析、把每个 host 的内联 PEM 写进私有 `Documents/<name>.pem`(0600)、`key`→纯文件名、原子写私有 `hosts.json` → 列表刷新出 host。**三类导入,按文件顶层内容自动判别 mode**(非按钮决定,与按钮不符时清晰提示):**①单个 host**(顶层 `host` 对象)→ **追加**(并入现有 hosts.json,按 name 去重覆盖同名);**②全局 hosts**(顶层 `hosts` 数组)→ **替换**整张列表;**③ASR 凭证**(顶层 `asr` 对象 `{provider,appid,token,resourceId}`,无 hosts)→ **只写 `asr.json`**,不动 hosts。三者可组合(全局文件可带 `asr`,各字段分别处理)。app 注册自定义扩展 `.xrhosts` + 自有 UTI `io.github.kevinfitzroy.xrealclient.hosts`(`UTExportedTypeDeclarations`,`LSHandlerRank=Owner`;**不抢 `public.json`**)。**用户不手输任何 host/key**,只导入一个文件。私有存储**结果形状不变**(hosts.json with bare-filename key),只是进来的"包"是内联单文件 |

> iOS 没有 `adb push 到任意 app 私有目录`这种能力(沙盒)。代客安装在 iOS 上**必然换实现**,但**契约形状(hosts.json/asr.json schema + 安全规则)不变**。模拟器开发期通道由 simctl 容器 copy 验证;**真机注入通道 = 分享单「Open in」+ 原生 Host 配置页文档选择器(两入口)+ 自含 `.xrhosts`**:导入逻辑(三类判别 append/replace/asr-only、解析/落盘/0600/原子写/列表刷新)已在模拟器验证(2026-05-31,经 `-importConfigPath` lever,走与 `open:` / 文档选择器同一条 `importConfig`;Host 配置页三按钮已就位、页面渲染 + 摘要已截图,但「按钮→picker→选文件」tap-through 因模拟器无 tap CLI 未端到端验),app 已带 doc-type/UTI 重装真机;**AirDrop「Open in XrealPOC」触发本身待真机验**(只有真机 AirDrop 能定 UTI 匹配是否生效)。曾是 iOS 客户端首要待解项,实现已落地。**这是平台实现变更(新增入口 + 三类导入),不 bump Contract version**(hosts.json/asr.json schema + 安全规则不变)。
