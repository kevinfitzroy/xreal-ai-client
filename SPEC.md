...
- **⭐ 行为契约(平台中立,iOS 实现者照这条做,内核/语言自选)**——满足以下可观测行为即合规,**不规定用哪个库**:
  ...
  > Android 用 xray-core dokodemo-door 实现这套;**iOS 已切 sing-box**(`direct` inbound + **route-action override**——sing-box 1.13 移除了 inbound 的 `override_*` 字段,改用 route rule 的 `override_address/override_port`,语义完全等价;`singbox-bridge/` gomobile 封官方 sing-box,vmess+vless 同走它)。换引擎动机:真机上 xray-core 的 vless+Reality(`xtls-rprx-vision`)隧道连上即停,而桌面 sing-box 同节点很稳(issue #46)。**只要满足上面 1–6 即合规**。平台落点见 §11。
