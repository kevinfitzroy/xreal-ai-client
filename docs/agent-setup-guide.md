...
- **⭐ 是否海外 host(从国内连它)**:**若是 → SSH-over-443 隧道是必选项,不是可选**。GFW 对海外 `:22` 的 DPI 干扰是持续且会演化的(今天好、明天卡、KEXINIT 被定点丢包),没有隧道这台 host 迟早连不上。**第 4.6 步必做**:确保该机器有 :443 的 vmess 服务、拿到 vmess 链接,第 5 步配进 `proxies`。国内 host / 局域网 host 不需要。
...
# ⚠️ 别跳过这步——这里漏装,重启后 maestro 不会回来且没人能远程唤醒(TK-ALIYUN 栽过这跟头)。
...
**c)** 这台 host 的 `vmess://` 链接留到**第 5 步**,配进 bundle 的 `proxies` 表 + host 的 `"proxy"` 字段。多跳内网 host(有 `via`)**不用**自己配 proxy——它蹭跳板的(归属规则见 SPEC §5.1)。
...
# 顶层两形态(app 都接受):①纯 host 列表(顶层数组,国内 host 无隧道用);②{proxies,hosts}(海外 host 带隧道)。
# —— 国内 / 无隧道 host:顶层数组即可 ——
...
# —— 海外 host:用 {proxies,hosts} 形态,host 加 "proxy" 指向第 4.6 步拿到的 vmess(SSH-over-443,§5.1)——
# cat > /tmp/xreal_hosts.json <<JSON
# {
#   "proxies": [
#     { "name": "${HOST_NAME}-443", "url": "vmess://...(第 4.6 步的链接)" }
#   ],
#   "hosts": [
#     { "name": "${HOST_NAME}", "addr": "<别名,不含真IP>", "host": "<host>", "port": 22,
#       "user": "<user>", "basePath": "${BASE}", "key": "${KEY_FILE}",
#       "proxy": "${HOST_NAME}-443",
#       "projects": [ { "session": "maestro", "name": "Maestro", "type": "maestro" } ] }
#   ]
# }
# JSON
# 多跳内网 host:host 加 "via":"<跳板名>" 但**不**加 proxy(蹭跳板的,§5.1 归属规则)。
