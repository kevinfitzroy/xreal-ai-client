...
> **要在这台 host 上建 Codex 项目?**(可选,见 orchestrator-CLAUDE.md §7.1)再验一下 `codex --version`;没装引导用户装 OpenAI Codex CLI。建 codex project 后**第一件事是验状态 hooks 真的 fire**(repo-local `.codex/config.toml` 的 hooks 可能在交互 session 不触发,codex#17532)——app 列表里那条状态会不会随发指令/停下而变(working↔waiting);不变就按 §7.1 回退到 `~/.codex/config.toml` 全局 hooks。
