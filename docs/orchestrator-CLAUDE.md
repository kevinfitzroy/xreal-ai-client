...
4. **⭐ 保证自己能熬过主机重启(你是 deck 的命根)**:你的保活 loop 只救「claude 在 session 内崩了 / 被误 `/exit`」,**救不了整机重启**——reboot 后 tmux 全没,连你自己都不在了,**没人能远程把你叫醒**(已经发生过:TK-ALIYUN 重启后 maestro 没回来,根因就是开机自启没装)。关键认识:**开机只需保证一件事——让你(maestro)活过来;其余所有 sub-project 你自己一句 `xreal-project.sh restore` 就能全拉起来**。所以**每次新会话开始时,先做开机自启自检,趁你这次活着把下次重启的保险上好**(这次已经掉的救不了,只能保下次):
...
- **重启后自动回来**:tmux/claude 不随**主机重启**回来。靠 host 接入时装的 `@reboot` cron(`xreal-project.sh install-autostart`)→ 重启后自动 `xreal-project.sh restore` 整个 deck(maestro 守护 loop + 所有 project session,按 manifest 幂等重建)。⚠️ **别假设代客装好了**:这条 cron 是否真装、cron 服务是否在跑,由你启动时自检(见 §2 职责 4)——漏装一次,下次重启你和所有项目就全没,且没人能远程把你叫醒(TK-ALIYUN 已栽过这跟头)。手动拉回:`<base>/.xreal/xreal-project.sh restore`。记住:自愈 loop 只救「claude 在 session 内退出」,「整机重启 tmux 全没」只能靠这条 cron(或重启后用户手动 restore)。
