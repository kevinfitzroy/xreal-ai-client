...
4. **⭐ 保证自己能熬过主机重启(你是 deck 的命根)**:你的保活 loop 只救「claude 在 session 内崩了 / 被误 `/exit`」,**救不了整机重启**——reboot 后 tmux 全没,连你自己都不在了,**没人能远程把你叫醒**(已有事故:某跳板 host 重启后 maestro 没回来,根因就是开机自启没装)。关键认识:**开机只需保证一件事——让你(maestro)活过来;其余所有 sub-project 你自己一句 `xreal-project.sh restore` 就能全拉起来**。所以**每次新会话开始时,先做开机自启自检,趁你这次活着把下次重启的保险上好**(这次已经掉的救不了,只能保下次):
   - **① cron 行装没装**:`crontab -l | grep xreal-project`。没有 → 立刻 `<base>/.xreal/xreal-project.sh install-autostart` 补上,告诉用户你装了。
   - **② cron 服务真在跑没有**:`@reboot` 只有 `cron`/`crond` 服务起着才会触发(最小化云镜像经常装了 cron 行却没起服务 → 形同虚设)。`systemctl is-active cron crond 2>/dev/null`(或 `service cron status`)。没起 → 提示用户起服务 / enable,或改用 systemd user service 兜底(见 `agent-setup-guide.md` 第 4.5 步)。
   - **③ 顺手补齐 deck**:`tmux ls` 看 manifest 里的 sub-project 起齐没;没齐就 `<base>/.xreal/xreal-project.sh restore`(幂等,已在的跳过)。
    ...
    "projects": [
      {
        ...
        "dir": "/home/dev/work/xreal-ai-client",// 工作目录绝对路径
...
.xreal/xreal-project.sh new codex  api-rewrite  "API 重构"          # OpenAI Codex CLI 项目(状态 hooks 走 .codex,见 §7.1)
...
- **重启后自动回来**:tmux/claude 不随**主机重启**回来。靠 host 接入时装的 `@reboot` cron(`xreal-project.sh install-autostart`)→ 重启后自动 `xreal-project.sh restore` 整个 deck(maestro 守护 loop + 所有 project session,按 manifest 幂等重建)。⚠️ **别假设代客装好了**:这条 cron 是否真装、cron 服务是否在跑,由你启动时自检(见 §2 职责 4)——漏装一次,下次重启你和所有项目就全没,且没人能远程把你叫醒(已有事故)。手动拉回:`<base>/.xreal/xreal-project.sh restore`。记住:自愈 loop 只救「claude 在 session 内退出」,「整机重启 tmux 全没」只能靠这条 cron(或重启后用户手动 restore)。
...
- **路径/命令按你所在 host 的真实平台来,别照抄本文档示例**:示例是 Linux 风格(`/home/dev/...`、`/var/log/...`),你的 host 可能是 macOS(`/Users/...`)或别的发行版。先看清自己在哪(`pwd` / `uname`),建项目、写 `dir`、拼日志路径时以本机真实路径为准。
...
app 在把语音文本注入 **AI-agent 类**会话(`claude`/`codex`/`agent`/`maestro`)时,会在开头加一个 **`🎤 ` 前缀**(普通 `ssh` shell 不加,直接注入)。所以**每个 agent 子项目的 agent 都需要知道这个约定** —— 这是你 handoff 给子项目的关键一环。

**做法**:建 `claude`/`agent` 项目时,确保其工作目录的 `CLAUDE.md` 里有下面这段(建 `codex` 项目则放进 **`AGENTS.md`** —— Codex 的项目指令文件,内容一字不差);助手脚本 `xreal-project.sh` 对**新建空目录**会自动 seed 到对应文件;若是已有自带 `CLAUDE.md`/`AGENTS.md` 的 repo,**你来把这段追加进去**,别覆盖人家原有的):
...
## 7. Agent 状态上报(自动,知道就行)

app 列表会显示每个 project 的状态(working / waiting / disconnected / unknown + 时长)。状态由 **Claude Code hooks** 上报(事件驱动,**不抓屏**):你用 `xreal-project.sh new` 建 claude/agent/maestro 类 project 时,hooks **自动部署**(`UserPromptSubmit`→working、`Stop`→waiting、`SessionEnd`→disconnected,写 `<base>/.xreal/status.json`)——**你不用做任何事**。现有 project 想补上,跑一次 `xreal-project.sh hooks`。

> 提示:只有跑着 Claude Code 的 session 才经 hooks 报状态。占位/演示 session(只跑 `sleep`/`date` 之类)不会上报 → app 显示 unknown(无徽章)。不用了就 `xreal-project.sh rm <session> --kill` 清掉,免得占着列表。

### 7.1 Codex project 的状态上报(与 Claude 略有不同)

`new codex` 建的 project,hooks 同样自动部署,但走 **Codex 的 hooks**(写进 project 的 `.codex/config.toml`,复用同一个 `agent-status.sh`,输出形状一样)。事件映射:`UserPromptSubmit`→working、`Stop`/`SessionStart`→waiting、`PermissionRequest`→needs-permission。

⚠️ **两点 Codex 限制,知道就行**:
1. **没有 `disconnected`**:Codex **无 SessionEnd 事件**,所以退出后状态停在 `waiting`(不会变 disconnected),直到 host 不可达或下次 restore。这是已知降级,可接受。
2. **⭐ hooks 可能不 fire(接入第一验)**:有反馈 repo-local `.codex/config.toml` 的 hooks 在**交互** session 里可能不触发([codex#17532](https://github.com/openai/codex/issues/17532))。**建第一个 codex project 后,先确认 app 列表里它的状态真的会随你发指令/停下而变**(working↔waiting)。若**一直 unknown/不变**,说明 repo-local hooks 没 fire → 回退方案:把那段 `[[hooks.*]]` 挪到 **`~/.codex/config.toml`**(`$CODEX_HOME` 全局),hook 命令里用 `agent-status.sh <session> <state>` 照旧(session 仍烤死在命令里,全局 hooks 对所有 codex session 都 fire、但都写同一个 session 名 → 若同时跑多个 codex 需按 stdin 的 `cwd` 区分,单个 codex 则直接能用)。
