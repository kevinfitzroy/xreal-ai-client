...
- **重启后自动回来**:tmux/claude 不随**主机重启**回来,但 host 接入时装了 `@reboot` cron(`xreal-project.sh install-autostart`)→ 重启后自动 `xreal-project.sh restore` 整个 deck(maestro 守护 loop + 所有 project session,按 manifest 幂等重建)。**你(Maestro)不用管**;真要手动拉回(如刚 reboot 还没自动起齐)跑一次 `<base>/.xreal/xreal-project.sh restore`。注意 maestro 的自愈 loop 只救「claude 在 session 内退出」,救不了「整机重启 tmux 全没」——后者靠这条 cron。
