...
| **支持 8BitDo F13/F14 这种非标键码作为 PTT 触发** | 没法在第三方 client 内部拦截 keycode 用作"我的按键",会跟 client 自己的快捷键打架 |
...
## 8. 为什么按键是 F13/F14,不是 Ctrl+Shift+V 之类

F13-F24 这些"扩展功能键" 在 Android 端定义存在(KEYCODE_F13 = 326 等),但**几乎没有应用占用**它们(因为大多数键盘只有 F1-F12)。也就是说,把 8BitDo Micro 的某个键配成 F13:

- 普通 App 收到 F13 → 啥都不做(没有 listener)
- 我们的 App → 在 `dispatchKeyEvent` 里拦截 → 触发 Voice Daemon
- **零冲突**

8BitDo Ultimate Software 官方支持发 F13-F24(已经核实)。Beam Pro 是否真的从蓝牙 HID 接收到 326/327 是 Stage A.1 实测项,fallback 是 Ctrl+Alt+1/2 组合键(几乎也不冲突)。
...
**一个 Android App,WebView 跑 xterm.js 当漂亮 terminal UI,Kotlin 用 sshj 直连云端 SSH,同 app 内一个 Voice Daemon 录音 → 豆包 ASR → 直接写 SSH outputStream。服务端零增量,只跑用户已有的 tmux + Claude Code。**
